...
#if defined(NDEBUG)
# error "Bitcoinbank cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Bitcoinbank Signed Message:\n";
...
{
    // Bitcoinbank: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Bitcoinbank: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Bitcoinbank: Special short-term limits to avoid 10,000 BDB lock limit:
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc1, 0xbb, 0xd1 }; // Bitcoinbank: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BitcoinbankMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BitcoinbankMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BitcoinbankMiner(CWallet *pwallet)
{
    printf("BitcoinbankMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bitcoinbank-miner");
    ...
    try { loop {
        ...
        printf("Running BitcoinbankMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BitcoinbankMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BitcoinbankMiner, pwallet));
}
